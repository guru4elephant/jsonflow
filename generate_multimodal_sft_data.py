...
def create_multimodal_pipeline(model_name: str, base_url: str, api_key: str, max_width: int = 800, max_height: int = 800) -> Pipeline:
    ...
    pipeline = Pipeline([
        ...
        # 步骤3: 调用Claude模型 - 使用MultimodalInvoker处理多模态输入
        MultimodalInvoker(
            model=model_name,
...
def main():
    ...
    parser.add_argument("--model", type=str, default="claude-3-7-sonnet-20250219", help="Claude模型名称")
    parser.add_argument("--api-key", type=str, help="API密钥")
    parser.add_argument("--base-url", type=str, default="http://ai.wenyue8.com:15588/v1", help="API基础URL")
    ...
    # 设置API密钥，如果未提供则从环境变量获取
    api_key = args.api_key or os.environ.get("OPENAI_API_KEY")
    if not api_key:
        print("错误: 未提供API密钥。请通过--api-key参数或OPENAI_API_KEY环境变量提供API密钥。")
        return 1
